...
def get_driver_from_retarget_dictionary_fixed_slider_range(
    ...
    if custom_slider:
        ...
        dr_info = control_rig_drivers_dict[expression_name]
    ...
    if dr_info:
        ...
        if variables:
            ...
            for v_name, v_data in variables.items():
                ...
                if type == 'TRANSFORMS':
                    ...
                    t.bone_target = v_data.get('bone_name', bone_name)
        ...
        # Add another variable for amplifying animation in realtime!
        var = dr.variables.new()
        ...
        if bone:  # and not expression or any([t in expression for t in ('{max_range}', '{min_range}')]):
        ...
        expression = expression.replace('{slider_max}', 'self.slider_max')\
            .replace('{slider_min}', 'self.slider_min')

        dr.expression = '(' + expression + ') * amp'
        return True, bone.name
    else:
...
def get_pose_bone_from_driver_dict(rig, arkit_shape_name):
    ...
    dr_info = control_rig_drivers_dict[arkit_shape_name]
...
def get_bone_animation_data(shape_key_name, c_rig):
    ...
    dr_info = control_rig_drivers_dict.get(shape_key_name)
    if not dr_info:
        # Get default dict
...
match_bones_asymmetry_dict = {
    ...
    103: {
        'head': [],
        'tail': [],
        'all': ['c_eye_lookat', 'c_eye.L', 'c_eye.R'],
    },
    104: {
        'head': [],
        'tail': [],
