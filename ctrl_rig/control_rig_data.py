...
def get_eye_2d_driver_dict(control_rig):
    ...
    mcp_max, mcp_min = get_pose_bone_range_from_limit_constraint(mocap2dtarget)
    mocap2d_L = control_rig.pose.bones.get('c_EyeLeft_slider2d')
    mcp_L_max, mcp_L_min = get_pose_bone_range_from_limit_constraint(mocap2d_L)
    mocap2d_R = control_rig.pose.bones.get('c_EyeRight_slider2d')
    mocap_switch = control_rig.pose.bones.get('c_SwitchLookAt_slider')
    switch_max, _switch_min = get_pose_bone_range_from_limit_constraint(mocap_switch)
    driver_dict['eyeLookUpLeft'] = {
        'expression': f'lerp(max(0, var/{mcp_max.x} + var_1/{mcp_L_max.x}), max(0, min(amp, wrld)), x / {switch_max.y})',
        'variables': {
            'var': {
            ...
            'x': {
                'bone_name': 'c_SwitchLookAt_slider',
                'transform_type': 'LOC_Y',
                'transform_space': 'LOCAL_SPACE',
    ...
    driver_dict['eyeLookDownLeft'] = {
        'expression': f'lerp(max(0, var/{mcp_min.x} + var_1/{mcp_L_min.x}), -min(0,max(-amp,wrld)), x / {switch_max.y})',
        'variables': {
            'var': {
                'bone_name': f'{mocap2dtarget.name}',
                'transform_type': 'LOC_X',
    ...
    driver_dict['eyeLookInLeft'] = {
        'expression': f'lerp(max(0, var/{mcp_min.y} + var_1/{mcp_L_min.y}), -min(0,max(-amp,wrld)), x / {switch_max.y})',
        'variables': {
            'var': {
                'bone_name': f'{mocap2dtarget.name}',
                'transform_type': 'LOC_Y',
    ...
    driver_dict['eyeLookOutLeft'] = {
        'expression': f'lerp(max(0, var/{mcp_max.y} + var_1/{mcp_L_max.y}), max(0,min(amp,wrld)), x / {switch_max.y})',
        'variables': {
            'var': {
                'bone_name': f'{mocap2dtarget.name}',
                'transform_type': 'LOC_Y',
